...
//to handle query
async function answerQuery(query, indexName = 'gemini-embed-768', options) {
    ...
    const q_match = search_res.matches?.filter(match => match.score && match.score >= scoreThreshold) || [];
    if (q_match.length === 0) {
        return {
